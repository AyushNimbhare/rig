...
export async function runPiAgentLoop(
  ...
  while (step < maxSteps) {
    ...
    // pi-style: if model returns final answer (no tool calls), stop
    ...
    // pi-style: append assistant turn with tool calls to chat history
    messages.push({
      role: "assistant",
    ...
    // Execute each requested tool call (pi-style loop)
    for (const toolCall of response.toolCalls) {
      ...
      // Execute tool (RIG-style, pi-influenced)
      try {
        ...
        // Track files changed (pi-style: write_patch tracks target files)
