/**
 * Fuzzy matching utilities — subsequence with word-boundary and consecutive bonuses.
 * Mirrors opencode/pi TUI fuzzy logic for "fuzzy picker like that".
 */
 ...
 * Tokens split by whitespace and slash must all match.
 */
...
export function highlightFuzzy(text: string, query: string): string {
  ...
  for (let i = 0; i < text.length; i++) {
    if (q < lowerQuery.length && lowerText[i] === lowerQuery[q]) {
      out += `\u001b[1m${text[i]}\u001b[22m`;
      q++;
    } else out += text[i];
  }
